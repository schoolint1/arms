    ...
    <div class="row mt-3">
        ...
        <div class="col-6">
            <div class="p-3 bg-light rounded shadow-sm">
                <h6 class="border-bottom border-gray pb-1 mb-0">Выбранные обучающиеся</h6>
                <div class="pt-2">
                    <div v-for="(itemUser, indexUser) in selectedUsers" v-on:click="onRemoveUser($event, itemUser)" class="a-user"><span>{{ itemUser.className }}</span> <span>{{ itemUser.name }}</span></div>
                </div>
            </div>
        </div>
    </div>
...
<script type="text/javascript">
    var app2 = new Vue({
        ...
        methods: {
            ...
            onRemoveUser: function(event, user) {
            ...
            onAddPlan: function(event) {
                ...
                for (let index in this.selectedUsers) {
                    su.push(this.selectedUsers[index].id);
                }
                var _self = this;
                $.ajax({
                    type: 'POST',
